    ...
    <div class="btn">
      ...
      <template v-if="order.orderState === 1">
        <XtxButton
          ...
          size="small"
        >立即付款</XtxButton>
        <XtxButton type="gray" size="small" @click="onCancel(order)">取消订单</XtxButton>
      </template>
      <!-- 待发货 -->
      ...
      <template v-if="order.orderState === 3">
        <XtxButton type="primary" size="small" @click="onConfirm(order)">确认收货</XtxButton>
        <XtxButton
          type="plain"
          size="small"
          @click="$router.push(`/member/checkout?orderId=${order.id}`)"
...
import { ref } from 'vue'
import { useConfirm, useCancel } from '../index.vue'
