        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="item in $store.getters['cart/validList']" :key="item.skuId">
              <td>
                <XtxCheckbox :modelValue="item.selected" @change="changeChecked(item.skuId,$event)" />
              </td>
              <td>
                <div class="goods">
                  ...
                  <div>
                    ...
                    <!-- 选择规格组件 -->
                    <CartSku :attrsText="item.attrsText" :skuId="item.skuId" @change="updateCartSku(item.skuId, $event)" />
                  </div>
                </div>
              </td>
              <td class="tc">
...
export default {
  ...
  setup() {
    ...
    // 选择全部
    const changeAll = (val) => {
      store.dispatch('cart/changeAll', val)
    }
    // 删除
    const deleteCart = async(skuId) => {
      try {
        await Confirm({ text: '您确认从购物车删除该商品吗？?' })
    ...
    const goCheckout = () => {
      if (store.getters['cart/selectedList'].length === 0) {
        return Message({ type: 'warn', text: '请选择至少一件商品' })
      }
      if (!store.state.user.profile.token) {
        return Confirm({ text: '结算前请先登录,您是否要去登录?' }).then(() => {
          // 点击确定跳转登录页面（全局导航守卫中做了处理）
